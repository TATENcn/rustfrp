---
name: rustfrp-core-dev
description: RustFRP 核心层开发约束 — 修改 crates/rustfrp-core/ 下任何 .rs 文件时自动注入
triggers:
  - "crates/rustfrp-core/**/*.rs"
---

# RustFRP Core 开发约束

> 触发条件：修改 `crates/rustfrp-core/` 下任何 `.rs` 文件时自动加载。
...
**崩溃重启**：上限 3 次，超过则停止重启 + 记录 ERROR 日志。
...
- 迁移只做增量（`ALTER TABLE ... ADD COLUMN`），不改已有列
