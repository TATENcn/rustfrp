...
### ADR-004：frps 二进制零侵入，服务端管理 Agent 可选部署

- **决策**：不修改原生 frps 二进制。在服务端提供可选的 `rustfrp-agent` 二进制，以子进程方式管理 frps，提供配置热更新和插件运行时。Agent 主动向控制面拉取配置模板（Pull 模式），控制面不做 Push。
- **否决方案**：修改 frps 源码、将 frps 编译进本项目、控制面直接推送配置到 frps
- **理由**：
  1. 协议兼容性——使用官方 frps 二进制确保 100% 兼容
  2. frps 独立升级——不受本项目发布周期限制
  3. Agent 可选——嵌入式/低配设备可不运行 Agent，下限不变
  4. Agent 宕机不影响穿透——frps 继续使用上次生成的 TOML 运行
  5. Agent 提供插件运行时——高配服务器可扩展监控、流量整形等能力
  6. 控制面宕机不影响穿透——Agent 持有配置缓存，独立决策

### ADR-005：配置下发走 Pull 模式，不做 Push

- **决策**：`rustfrp-agent` 主动向控制面拉取配置模板，而非接收控制面的推送。控制面提供配置模板仓库（只读 API），Agent 按需拉取、本地校验后生成 `frps.toml`。
- **否决方案**：控制面直接推送配置到 frps、gRPC 双向流实时同步、SSH 批量下发
- **理由**：
  1. 控制面宕机 → Agent 继续用本地缓存配置，不影响穿透
  2. Agent 宕机 → frps 继续运行，不产生孤儿连接
  3. 配置冲突由 Agent 本地解决（GitOps 风格），不依赖中心协调
  4. 简单实现——Agent 侧一个 HTTP GET + TOML 生成即可，无需双向 RPC
