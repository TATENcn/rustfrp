...
impl FrpsProfile {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.name.trim().is_empty() {
            return Err(CoreError::MissingRequiredField("name".into()));
        }

        // 服务端地址：合法 IP 或域名
        if !is_valid_host(&self.server_addr) {
            return Err(CoreError::InvalidIpAddress(self.server_addr.clone()));
        }

        // 端口范围
        if self.server_port == 0 {
            return Err(CoreError::InvalidPort("server_port cannot be 0".into()));
        }
...
impl LocalProxy {
    ...
    pub fn validate(&self) -> Result<()> {
        // 名称必填
        if self.name.trim().is_empty() {
            return Err(CoreError::MissingRequiredField("proxy name".into()));
        }

        // 本地 IP
        if !is_valid_host(&self.local_ip) {
            return Err(CoreError::InvalidIpAddress(self.local_ip.clone()));
        }

        // 端口范围
        if self.local_port == 0 {
            return Err(CoreError::InvalidPort("local_port cannot be 0".into()));
        }

        // remote_port：若提供则不能为 0
        if let Some(port) = self.remote_port {
            if port == 0 {
                return Err(CoreError::InvalidPort("remote_port cannot be 0".into()));
            }
        }
        ...
        if let Some(ref plugin) = self.plugin_config {
            if plugin.get("type").and_then(|v| v.as_str()).unwrap_or("").is_empty() {
                return Err(CoreError::ConfigValidation(
...
impl BindingRule {
    ...
    pub fn validate(&self) -> Result<()> {
        if self.profile_id <= 0 {
            return Err(CoreError::ConfigValidation("profile_id must be > 0".into()));
        }
        if self.proxy_id <= 0 {
            return Err(CoreError::ConfigValidation("proxy_id must be > 0".into()));
        }
        Ok(())
    }
}
