...
fn ensure_meta_table(conn: &Connection) -> Result<()> {
    ...
    .map_err(|e| CoreError::DatabaseMigration(format!("Failed to create _schema_meta: {e}")))?;
    Ok(())
}
...
fn get_current_version(conn: &Connection) -> Result<i32> {
    ...
    version
        .parse::<i32>()
        .map_err(|_| CoreError::DatabaseMigration(format!("Invalid schema version: {version}")))
}
...
fn apply_migration(conn: &Connection, version: i32) -> Result<()> {
    let tx = conn
        ...
        .map_err(|e| CoreError::DatabaseMigration(format!("Failed to begin transaction: {e}")))?;

    match version {
        1 => migrate_v1(&tx)?,
        2 => migrate_v2(&tx)?,
        _ => {
            return Err(CoreError::DatabaseMigration(format!(
    ...
    .map_err(|e| CoreError::DatabaseMigration(format!("Failed to update version number: {e}")))?;

    tx.commit()
        .map_err(|e| CoreError::DatabaseMigration(format!("Migration commit failed: {e}")))?;
...
fn migrate_v1(tx: &rusqlite::Transaction) -> Result<()> {
    ...
    .map_err(|e| CoreError::DatabaseMigration(format!("Failed to create frps_profile table: {e}")))?;
    ...
    .map_err(|e| CoreError::DatabaseMigration(format!("Failed to create local_proxy table: {e}")))?;
    ...
    .map_err(|e| CoreError::DatabaseMigration(format!("Failed to create binding_rule table: {e}")))?;
...
fn migrate_v2(tx: &rusqlite::Transaction) -> Result<()> {
    ...
    .map_err(|e| {
        ...
        CoreError::DatabaseMigration(format!("Failed to add plugin_config column: {e}"))
    })?;
...
pub fn verify_checksum(conn: &Connection) -> Result<()> {
    ...
    match stored {
        Some(stored_hash) if stored_hash != schema_hash => {
            Err(CoreError::DatabaseMigration(format!(
        ...
        None => {
            ...
            .map_err(|e| CoreError::DatabaseMigration(format!("Failed to store checksum: {e}")))?;
...
fn compute_schema_hash(conn: &Connection) -> Result<String> {
    let mut stmt = conn
        ...
        .map_err(|e| CoreError::DatabaseMigration(format!("Failed to read schema: {e}")))?;

    let schemas: Vec<String> = stmt
        .query_map([], |row| row.get(0))?
        .collect::<std::result::Result<Vec<_>, _>>().map_err(CoreError::DatabaseQuery)?;
