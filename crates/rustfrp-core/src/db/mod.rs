    ...
    pub async fn open(path: &str) -> Result<Self> {
        ...
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| CoreError::DatabaseConnection(format!("Failed to create database directory: {e}")))?;
        }

        let conn = Connection::open(&db_path)
            .map_err(|e| CoreError::DatabaseConnection(format!("Failed to open database: {e}")))?;

        // 启用 WAL 模式（允许并发读）
        conn.pragma_update(None, "journal_mode", "WAL")
            .map_err(|e| CoreError::DatabaseConnection(format!("Failed to enable WAL: {e}")))?;

        // 启用外键约束
        conn.pragma_update(None, "foreign_keys", "ON")
            .map_err(|e| CoreError::DatabaseConnection(format!("Failed to enable foreign keys: {e}")))?;
