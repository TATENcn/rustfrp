...
impl Database {
    ...
    pub async fn get_profile(&self, id: i64) -> Result<FrpsProfile> {
        ...
        .map_err(|e| match e {
            rusqlite::Error::QueryReturnedNoRows => CoreError::RecordNotFound {
                table: "frps_profile".into(),
                id,
            },
            other => CoreError::DatabaseQuery(other),
        })
    }
    ...
    pub async fn update_profile(&self, profile: &FrpsProfile) -> Result<()> {
        let id = profile.id.ok_or_else(|| {
            CoreError::ConfigValidation("Profile id is required for update".into())
        })?;
    ...
    pub async fn delete_profile(&self, id: i64) -> Result<()> {
        ...
        let affected = conn
            .execute(
                "DELETE FROM frps_profile WHERE id = ?1",
                params![id],
            )
            .map_err(CoreError::DatabaseQuery)?;
...
mod tests {
    ...
    async fn test_insert_and_get_profile() {
        ...
        let id = db.insert_profile(&sample_profile("测试服务器")).await.unwrap();
        assert!(id > 0);
