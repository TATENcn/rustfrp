...
impl PluginManager {
    ...
    pub async fn load_all(&self) -> Result<Vec<String>> {
        ...
        std::fs::create_dir_all(&self.plugins_dir)
            .map_err(|e| CoreError::PluginLoad(format!("Failed to create plugin directory: {e}")))?;

        let mut loaded = Vec::new();

        let entries = std::fs::read_dir(&self.plugins_dir)
            .map_err(|e| CoreError::PluginLoad(format!("Failed to read plugin directory: {e}")))?;

        for entry in entries {
            let entry = entry.map_err(|e| {
                CoreError::PluginLoad(format!("Failed to read directory entry: {e}"))
            })?;
    ...
    pub async fn load_plugin(&self, dir: &Path) -> Result<String> {
        ...
        let manifest_json = std::fs::read_to_string(&manifest_path)
            .map_err(|e| CoreError::PluginLoad(format!("Failed to read manifest.json: {e}")))?;

        let manifest = PluginManifest::from_json(&manifest_json)
            .map_err(|e| CoreError::PluginValidation(format!("manifest.json parse failed: {e}")))?;

        // 校验 manifest
        manifest
            .validate()
            .map_err(|errors| CoreError::PluginValidation(errors.join("; ")))?;
        ...
        // 检查插件是否已存在
        {
            ...
            if instances.contains_key(&name) {
                return Err(CoreError::PluginLoad(format!("Plugin '{name}' is already loaded")));
            }
        }
    ...
    pub async fn unload_plugin(&self, name: &str) -> Result<()> {
        ...
        let instance = instances
            .get(name)
            .ok_or_else(|| CoreError::PluginUnload(format!("Plugin '{name}' does not exist")))?;
        ...
        if !matches!(
            ...
            return Err(CoreError::PluginUnload(format!(
    ...
    async fn check_dependencies(&self, manifest: &PluginManifest) -> Result<()> {
        ...
        for dep in &manifest.dependencies {
            if !instances.contains_key(dep) {
                return Err(CoreError::PluginValidation(format!(
