...
impl ProcessGuard {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        std::fs::create_dir_all(&self.log_dir)
            .map_err(|e| CoreError::ProcessStart(format!("Failed to create log directory: {e}")))?;
    ...
    async fn spawn_child(&self) -> Result<()> {
        ...
        if !config_path.exists() {
            return Err(CoreError::ProcessStart(format!(
                "Config file does not exist: {}",
                config_path.display()
            )));
        }

        let stdout_file = OpenOptions::new().append(true).create(true).open(log_dir.join("frpc.log"))
            .map_err(|e| CoreError::ProcessStart(format!("Failed to open frpc.log: {e}")))?;

        let stderr_file = OpenOptions::new().append(true).create(true).open(log_dir.join("frpc_err.log"))
            .map_err(|e| CoreError::ProcessStart(format!("Failed to open frpc_err.log: {e}")))?;
        ...
        let child = Command::new(FRPC_BINARY)
            ...
            .kill_on_drop(true)
            .spawn()
            .map_err(|e| CoreError::ProcessStart(format!("Failed to start frpc: {e}. Make sure frpc is installed and in PATH")))?;
    ...
    fn spawn_watchdog(&self, ready_rx: tokio::sync::oneshot::Receiver<()>) {
        ...
        tokio::spawn(async move {
            ...
            if count < MAX_RESTART_COUNT {
                ...
                // 重新启动
                let stdout_file = match OpenOptions::new().append(true).create(true).open(log_dir.join("frpc.log")) {
                    Ok(f) => f,
                    Err(e) => {
                ...
                let stderr_file = match OpenOptions::new().append(true).create(true).open(log_dir.join("frpc_err.log")) {
                    Ok(f) => f,
                    Err(e) => {
    ...
    pub async fn reload(&self) -> Result<()> {
        ...
        let child = guard
            .as_ref()
            .ok_or_else(|| CoreError::ProcessCommunication("frpc is not running".into()))?;

        let pid = child.id().ok_or_else(|| {
            CoreError::ProcessCommunication("Failed to get frpc PID".into())
        })?;
        ...
        #[cfg(unix)]
        {
            ...
            kill(Pid::from_raw(pid as i32), Signal::SIGHUP)
                .map_err(|e| CoreError::SignalError(format!("SIGHUP send failed: {e}")))?;
